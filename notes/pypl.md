...
This document summarizes the steps taken to prepare the `landmapy` package for its first release on PyPI.
...
- **The `build/` Conflict**: Discovered that a local directory named `build/` in the repository root causes `python -m build` to fail (as it tries to execute the local folder instead of the library).
    - **Fix**: Always delete `build/` and `landmapy.egg-info/` before running a fresh build.
...
1. Verify the project name (e.g., `landmapy`) isn't already taken by someone else on PyPI.
