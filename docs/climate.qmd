...
- Harmonize all datasets and build fuzzy logic.
  ...
  - see
[notes on skfuzzy](https://github.com/byandell-envsys/landmapyr/blob/main/notes.qmd#fuzzy-logic-model-27-feb-2025)
- Spruce up graphics, possibly using
  - slider widget (see code below in [Climate model data](#climate-model-data) section)
  - matplotlib with multiple subplots (see [6 Mar notes](https://github.com/byandell-envsys/landmapyr/blob/main/notes.qmd#notes-6-mar-2025))
...
According to the USDA Natural Resources Convervations Service: Plant Guide:
[Little Bluestem](https://plants.usda.gov/DocumentLibrary/plantguide/pdf/pg_scsc.pdf)
"is a tallgrass prairie increaser and mixed prairie decreaser.
Little bluestem typically occurs on dry
upland sites, especially on ridges, hilltops, and steep slopes.
It also occurs on limey sub-irrigated sites and in prairie fens.
It is found in areas receiving 10 to 60 inches of mean annual precipitation and plant hardiness zones 3 to 9."
[Plant profile page](https://plants.usda.gov/plant-profile/SCHIZ4).

[Big Bluestem](https://www.nrcs.usda.gov/sites/default/files/2022-09/BigBluestem.pdf)
"is climatically adapted throughout the
Midwest and Northeast on moderately well drained
through excessively well drained soils.
It is adapted to a range of other soil limitations
such as shallow depth, low pH, and low fertility."
[Plant profile page](https://plants.usda.gov/plant-profile/ANGE).

While I could use
[GBIF](https://www.gbif.org/)
to find crowd-sourced records,
these two species are widely found across North America.
For more on using GBIF with Python, see

- [GBIF Class Notes](https://github.com/earthlab-education/habitat-suitability-byandell/blob/main/notes.md#gbif-review-25-feb-2025)
...
[landmapyr](https://github.com/byandell-envsys/landmapyr)
package
  - [siberian_crane.qmd](https://github.com/earthlab-education/species-distribution-coding-challenge-byandell/blob/main/siberian_crane.qmd) (use of `gbif.py` module)
...
- [PAD-US](https://www.usgs.gov/programs/gap-analysis-project/science/pad-us-data-overview)
  - [PADUS4_0_State_SD_GDB](https://www.sciencebase.gov/catalog/item/652d4f80d34e44db0e2ee45c#:~:text=PADUS4_0_State_SD_GDB) (834KB zip)
- Duan K, Sun G, Sun S et al. (2016)
Divergence of ecosystem services in U.S. National Forests and Grasslands under a changing climate. Sci Rep 6, 24441.
[DOI: 10.1038/srep24441](https://doi.org/10.1038/srep24441)
- Brown RN, Percivalle C, Narkiewicz S, DeCuollo S (2010)
Relative Rooting Depths of Native Grasses and Amenity Grasses with Potential for Use on Roadsides in New England. HortScience 45: 393-400.
[DOI: 10.21273/HORTSCI.45.3.393](https://doi.org/10.21273/HORTSCI.45.3.393)
...
#| fig-title: "Buffalo and Oglala Grasslands with Pine Ridge (red) and SD/NE (black)"
plot_gdf_state(buffalo_gdf, aiannh=True)
```
...
```{python}
#| label: fig-soil
#| fig-title: "Percent Sand in Soil overlaid by Grasslands"
...
```{python}
#| label: fig-srtm
#| fig-title: "Elevation overlaid by Grasslands"
plot_gdf_da(buffalo_gdf, srtm_da, cmap='terrain')
```

```{python}
#| label: fig-slope
#| fig-title: "Slope overlaid by Grasslands"
...
```{python}
#| label: fig-slopeclip
#| fig-title: "Slope clipped by Grasslands"
...
Project precipation `pr` under representative concentration pathway scenarios `rcp45` and `rcp85` for years `2006-2035` and `2036-2065`.
...
```{python}
#| label: fig-maca
#| fig-title: "RCP 4.5 vs 8.5 for 2025"
# 0 = `rcp85`, 1 = `rcp45`
plot_das(maca_rcp_2025_da, gdf = buffalo_gdf, onebar = False)
...
```{python}
#| label: fig-maca45
#| fig-title: "RCP 4.5 2006-2065"
#| fig-cap: "Every 5 years wrapped around."
...
```{python}
#| label: fig-dasnow
#| fig-title: "Mean Change by RCP for 2006-2035"
plot_das(maca_now_da, onebar=False)
```

```{python}
#| label: fig-dasfut
#| fig-title: "Mean Change by RCP for 2036-2065"
...
Get mean across 30-year intervals for two eras
and two RCP levels.
...
Reproject sand and maca DAs with slope,
and clip to grassland boundaries.
...
```{python}
#| label: fig-slopesandclip
#| fig-title: "Slope and Sand Clipped to Grasslands"
slope_da_clip.plot()
sand_da.plot()
```
