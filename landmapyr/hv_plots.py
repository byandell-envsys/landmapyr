...
def hvplot_tract_gdf(place_tract_gdf):
    ...
    place_hv = place_tract_gdf.to_crs(ccrs.Mercator()).hvplot(
        line_color="orange",
        fill_color=None,
        crs=ccrs.Mercator(),
        tiles="EsriImagery",
...
def hvplot_matrix(model_df):
    ...
    matrix_hv = hvplot.scatter_matrix(
        model_df[["mean_patch_size", "edge_density", "log_asthma"]]
    )
...
def hvplot_delta_gdf(delta_gdf, width=600, height=300):
    ...
    delta_hv = (
        ...
        .hvplot(alpha=0.2, fill_color="white", tiles="EsriImagery", crs=ccrs.Mercator())
