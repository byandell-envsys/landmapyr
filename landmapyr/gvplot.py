...
def gvplot_ndvi_index(place_gdf, index="asthma"):
    ...
    Returns:
        gvplot
    """
    gvplot_ndvi = gvplot_chloropleth(
        place_gdf, color=index, cmap="Blues", title=index.title()
    ) + gvplot_chloropleth(
        place_gdf, color="edge_density", cmap="Greens", title="Edge Density"
    )
...
def gvplot_resid(model_df):
    ...
    resid_gv = (
        gvplot_chloropleth(
            model_df, color="resid", cmap="RdBu", title="Residuals for Asthma"
        ).redim.range(resid=(-0.3, 0.3))
