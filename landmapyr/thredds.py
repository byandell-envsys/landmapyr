    ...
    for site_name, site_gdf in sites.items():
        for scenario in scenarios:
            for climate in climates:
                ...
                for year in range(year_min, year_max, 5):
                    ...
                    maca_da_year = maca_da_year.assign_coords(
                        lon=(
                            "lon",
                            [
                                convert_lonlat(lon_val)
                                for lon_val in maca_da_year.lon.values
                            ],
                        ),
                        lat=(
                            "lat",
                            [
                                convert_lonlat(lat_val)
                                for lat_val in maca_da_year.lat.values
                            ],
                        ),
                    )
                    maca_da_year = maca_da_year.rio.set_spatial_dims(
                        x_dim="lon", y_dim="lat"
                    )
                    # Clip bounds.
                ...
                info.append(
                    dict(site_name=site_name, scenario=scenario, climate=climate)
                )
