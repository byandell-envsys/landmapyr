...
def process_metadata(city_files):
    ...
    uri_groups = [
        uri_re.search(city_file.full_name).groupdict() for city_file in city_files
    ]
...
def process_bands(city_gdf, raster_df):
    ...
    bands = {"B02": "red", "B03": "green", "B04": "blue", "B05": "nir"}
    ...
    for tile_id, tile_df in raster_df.groupby("tile_id"):
        ...
        cloud_mask = process_cloud_mask(fmask_file, city_gdf, [1, 2, 3, 5])
        ...
        for band_id, row in tile_df.groupby("band_id"):
            if band_id in bands:
                ...
                # Process band
                band_da = process_image(row.file.values[0], city_gdf)
    ...
    # Merge all tiles
    city_merged_das = {
        band_name: merge_arrays(das) for band_name, das in city_das.items()
    }
...
def da2gdf(data_array):
    ...
    shapes_gen = shapes(
        data_array.values, mask=mask.values, transform=data_array.rio.transform()
    )

    # Convert shapes to a GeoDataFrame
    geoms = list(shapes_gen)
    gdf = gpd.GeoDataFrame.from_features(
        [{"geometry": geom, "properties": {"value": value}} for geom, value in geoms]
    )
