...
def plot_usgs_map(site_id, site_name, latitude, longitude):
    ...
    # Plot the point
    gdf_3857.plot(
        ax=ax, color="blue", markersize=100, edgecolor="white", linewidth=2, zorder=5
    )
...
def get_usgs_data(
    ...
    df = nwis.get_record(
        sites=site_id, parameterCd=parameters, start=start_date, end=end_date
    )
    ...
    df_daily = df[target_param].resample("D").mean()
